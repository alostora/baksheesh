<?php

return [

    'page_title' => 'طلبات السحب',

    'filter' => 'فلتر',

    'query_string' => 'كلمة البحث',

    'select' => 'اختر',

    'client' => 'عميل',

    'amount' => 'المبلغ',

    'status' => 'حالة الطلب',

    'created_at' => 'تاريخ الاضافة',

    'action_at' => 'تاريخ الحدث',

    'action_by' => 'الحدث بواسطة',

    'reference_code' => 'الرقم المرجعي',

    'bank_transfer_number' => 'رقم الحوالة البنكية',

    'admin_notes' => 'ملاحظات الاداره',

    'client_notes' => 'ملاحظات العميل',

    'refuse_reasone' => 'سبب الرفض',

    //operatoins
    "operations" => "اجراءات",

    "create" => "اضافة طلب سحب",

    "update" => "تعديل بيانات طلب السحب",

    "submit" => "حفظ",

    "close" => "اغلاق",

    "search" => "بحث",

    "empty" => "فارغ",

    "current_status" => "الحالة الحالية",

    "active" => "نشط",

    "inactive" => "غير نشط",

    "change_withdrawal_request_status" => "تغير حالة طلب السحب",

    "total" => "الاجمالي",
    "pending" => "انتظار",
    "accepted" => "تمت الموافقه",
    "implemented" => "تم التضمين",
    "refused" => "مرفوض",
    "unexecutable" => "تعذر التنفيذ",



    "print_report_head" => "نحيط سيادتكم علما بان  ",

    "print_count_all" => "اجمالي عدد طلبات السحب لجميع العملاء   ",
    "print_count_pending" => "اجمالي عدد طلبات السحب قيد الانتظار لجميع العملاء ",
    "print_count_accepted" => "اجمالي عدد طلبات السحب المقبولة لجميع العملاء ",
    "print_count_refused" => "اجمالي عدد طلبات السحب المرفوضة لجميع العملاء ",
    "print_count_unexecutable" => "اجمالي عدد طلبات السحب التي تعذر تنفيذها لجميع العملاء ",

    "print_sum_all" => "اجمالي مجموع طلبات السحب لجميع العملاء ",
    "print_sum_pending" => "اجمالي مجموع طلبات السحب قيد الانتظار لجميع العملاء ",
    "print_sum_accepted" => "اجمالي مجموع طلبات السحب المقبولة لجميع العملاء ",
    "print_sum_refused" => "اجمالي مجموع طلبات السحب المرفوضة لجميع العملاء ",
    "print_sum_unexecutable" => "اجمالي مجموع طلبات السحب التي تعذر تنفيذها لجميع العملاء ",



    "print_client_count_all" => "اجمالي عدد طلبات السحب للعميل   ",
    "print_client_count_pending" => "اجمالي عدد طلبات السحب قيد الانتظار للعميل ",
    "print_client_count_accepted" => "اجمالي عدد طلبات السحب المقبولة للعميل ",
    "print_client_count_refused" => "اجمالي عدد طلبات السحب المرفوضة للعميل ",
    "print_client_count_unexecutable" => "اجمالي عدد طلبات السحب التي تعذر تنفيذها للعميل ",

    "print_client_sum_all" => "اجمالي مجموع طلبات السحب للعميل ",
    "print_client_sum_pending" => "اجمالي مجموع طلبات السحب قيد الانتظار للعميل ",
    "print_client_sum_accepted" => "اجمالي مجموع طلبات السحب المقبولة للعميل ",
    "print_client_sum_refused" => "اجمالي مجموع طلبات السحب المرفوضة للعميل ",
    "print_client_sum_unexecutable" => "اجمالي مجموع طلبات السحب التي تعذر تنفيذها للعميل ",



];
